  ...
  async function loadFiche() {
    ...
    if (data) {
      ...
      if (Array.isArray(parsed.formats_couleurs)) {
        while (parsed.formats_couleurs.length < 8) parsed.formats_couleurs.push({ l: '', h: '', nb: '', kg_couleur: '' })
      ...
      <div ref={printRef} style={{ maxWidth: 900, margin: '24px auto', background: 'white', borderRadius: 8, boxShadow: '0 2px 12px rgba(0,0,0,0.1)', padding: '20px 24px 32px' }}>
            ...
            {f.formats_couleurs.map((fc, i) => (
              <div key={i} style={{ border: '1px solid #eee', borderRadius: 4, padding: '6px 8px' }}>
                ...
                <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                  <Input type="number" value={fc.kg_couleur} placeholder="0.0" style={{ width: 50 }}
                    onChange={v => { const arr = [...f.formats_couleurs]; arr[i] = {...arr[i], kg_couleur: v}; set('formats_couleurs', arr) }} />
                  <span style={{ fontSize: 10, color: '#888' }}>Kg/couleur</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ padding: '12px', border: '1px solid #ddd', borderTop: 'none' }}>
          ...
          <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start', marginBottom: 10, flexWrap: 'wrap' }}>
            <Field label={t.laize_papier} style={{ minWidth: 120 }}>
